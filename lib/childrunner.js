...
function RemoteSession(host, session) {
  this.host = host;
...
RemoteHost.prototype = {
  ...
  createSession: function(profile, options) {
    id = 'session-' + uuid.v4();
    profile = profile || '~/workspace/gaia/profile';
...
ChildRunner.prototype = {
  ...
  _buildIPCCallback: function(uniqueId) {
    ...
    return function() {
      ...
      args[0] = errorIPC.serialize(args[0]);
  ...
  /**
   * Handle marionette related errors.
   *
   * @param {Object} error serialized error object.
   * @param {Function} callbcak [Error err]
   */
  handleError: function(error) {
    error = errorIPC.deserialize(error);
  },
  ...
  cleanup: function(callback) {
    ...
    var next = function next() {
      if (--pending === 0) {
        this.process.kill();
        this.process.once('exit', callback);
      }
    }.bind(this);
  ...
  createHost: function() {
    ...
    return this.host.module.createHost().
      then(function(host) {
        this.hosts[id] = new RemoteHost(this.rpc, host, {
          profileBuilder: this.profileBuilder,
          host: this.host,
          profileBase: this.profileBase
        });
        this.rpc.register(id, this.hosts[id]);
        return id;
      }.bind(this));
  },
